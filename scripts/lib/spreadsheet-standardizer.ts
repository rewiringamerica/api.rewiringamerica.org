...
export class SpreadsheetStandardizer {
  ...
  recordToStandardValues(
    state: string,
    record: CollectedStateIncentive,
...
function isPlausibleLowIncomeRow(record: CollectedStateIncentive) {
  if (
  ...
  if (
    record.short_description?.en !== undefined &&
