...
import { _private } from './incentive-spreadsheet-to-json';
...
test('correct row to record transformation', async tap => {
    ...
    want: Record<string, string | number | object>;
  }[] = [
      {
        ...
        want: {
          ...
          item_type: 'rebate',
          program: 'va_appalachianPowertakeChargeVirginiaEfficientProductsProgram',
          amount: {
            type: 'dollar_amount',
            number: 50,
          },
  ...
  for (const tc of testCases) {
    ...
    tap.matchOnly(_private.recordToJson('va', renamed), tc.want);
  }
});
