...
import SOLAR_PRICES from '../data/solar_prices.json' assert { type: 'json' };
import STATE_MFIS from '../data/state_mfi.json' assert { type: 'json' };
...
export default function calculateIncentives(
  ...
  if (!ami_result || !ami_result.location || !ami_result.ami || !ami_result.calculations) {
  ...
  const solarTaxCredit = SOLAR_PRICES[location?.state_name]?.tax_credit;
  const stateMFI = STATE_MFIS[location?.state_name];
  ...
  for (const item of INCENTIVES) {
    ...
    if (item.item_type === 'solar_tax_credit') {
      representative_amount = solarTaxCredit;
    }
  ...
  for (const item of eligibleIncentives) {
    if (item.type === 'pos_rebate') {
      ...
      // FIXME: checking with @derek, do we do this on purpose?
      if (item.item_type == 'ev_charger_credit') {
        continue;
      }
