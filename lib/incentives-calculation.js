...
export default function calculateIncentives(
  ...
  if (!OWNER_STATUSES.has(owner_status)) {
    throw new ErrorWrapper("Unknown owner_status");
  }

  if (!TAX_FILINGS.has(tax_filing)) {
    throw new ErrorWrapper("Unknown tax_filing.");
  }

  if (isNaN(household_income) || household_income < 0 || household_income > 100000000) {
    throw new ErrorWrapper("Invalid household_income. Must be >= 0 and <= 100000000");
  }

  if (isNaN(household_size) || household_size < 1 || household_size > 8) {
    throw new ErrorWrapper('Invalid household size. Must be a number between 1 and 8.');
  }
  ...
  if (isNaN(solarSystemCost) || isNaN(stateMFI?.TOTAL)) {
    throw new ErrorWrapper('Invalid state id provided. Must be US state code or DC.')
  }
  ...
  for (const item of eligibleIncentives) {
    if (item.type === 'pos_rebate') {
      ...
      throw new ErrorWrapper(`Unknown item_type: ${item.type}`);
    }
  }
