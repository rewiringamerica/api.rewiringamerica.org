...
export default function calculateIncentives(
  ...
  if (isNaN(household_size) || household_size < 1 || household_size > 8) {
    throw new Error('Invalid household_size. Must be a number between 1 and 8.');
  }
  ...
  const sortedIncentives = _.orderBy(calculatedIncentives, ['amount_type', 'amount'], ['desc', 'desc']);
  ...
  return {
    ...
    pos_rebate_incentives: sortedIncentives.filter(item => item.type === 'pos_rebate'),
    tax_credit_incentives: sortedIncentives.filter(item => item.type === 'tax_credit'),
  };
}
