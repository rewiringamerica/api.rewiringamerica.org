...
const TAX_BRACKETS = JSON.parse(fs.readFileSync('./data/tax_brackets.json', 'utf-8'));
...
export default function estimateTaxAmount(
  ...
  const tax_brackets = TAX_BRACKETS.filter(row => filing_status === row.filing_status);

  // Get the Standard Deduction for filing status
  const standardDeductionResult = tax_brackets.find(row => {
    return household_income <= row.income_max && household_income >= row.income_min;
  });
